...
for row in results:
    ...
    parts = row.name.split('_', 2)  # 以第二个 _ 分割
    if len(parts) > 2:
        set_tail.append(parts[2])
#去重
set_tail = list(set(set_tail))
#逐行打印set_tail
# shoulder chest  leg  glove  boot shoulder_d chest_d  leg_d  glove_d  boot_d 10种部位 +或head 
# rb lt  ch  pl  4种护甲类型 

hujias = ['rb', 'lt', 'ch', 'pl']
buweis = ['shoulder', 'chest', 'leg', 'glove', 'boot',  'head','torso','pants','shoes']
for set_tail in set_tail:
    # 4*5=20种组合
    # 拼接sql语句在item_armors表中查询以hujia为开头的数据模糊查询以set_tail为结尾的数据
    # 查询结果存入mixset中
    mixset = []
    for hujia in hujias:
        for buwei in buweis:
            sql=f'''select name from [XMLParser2].[dbo].[item_armors] where name = '{hujia}_{buwei}_{set_tail}' '''
            cursor.execute(sql)
            #print(sql)
            #print(cursor.fetchall())
            mixset += cursor.fetchall()       
        #print(len(mixset))
    

    #将mixset 追加文件setitems.txt中
    with open('setitems.txt', 'a') as f:
        for row in mixset:
            f.write(row.name + '\n')
        f.write('\n')
    #逐行打印mixset
